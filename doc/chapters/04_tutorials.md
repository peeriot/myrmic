...
- [First Steps with the Signal Layer](./04_tutorials/03_first-steps-signal-layer.md) - Build a pipeline that produces a value and a cell that reads it, then move the same cell from a Linux machine onto an ESP32-C6 with one command.
...
- [Scaling Your Application](./04_tutorials/04_scaling.md) - Let root cells spawn grow-beds and sensors at runtime from a dashboard button, pair them by name, and see who brings a lost child back when a node dies.
