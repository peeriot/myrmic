...
async fn relay1_sink_task(out: linux_gpio_shim::LinuxOutputPin) {
    ...
    let mut ticker = IntervalStream::new(interval(Duration::from_millis(100u64)));
    loop {
        ticker.next().await;
        let now = signal_layer_linux_rt::time::now_millis();
...
pub fn spawn_sources(spawner: &Spawner, peripherals: BoardPeripherals) {
    ...
    tokio::spawn(bme280_task(i2c0_mutex.clone(), peripherals.fan1_out));
    tokio::spawn(relay1_sink_task(peripherals.relay1_out));
}
...
async fn main() {
    ...
    log::info!("pipeline `linux-outlets` starting");
    ...
    println!("Pipeline `linux-outlets` running. Press Ctrl-C to stop.");
    log::info!("pipeline `linux-outlets` running");
    tokio::signal::ctrl_c()
        .await
